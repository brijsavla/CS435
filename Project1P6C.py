...
def AVLfindMaxIter(root):
  curr = root
  while curr.right != None:
    curr = curr.right
  return curr
#Finding the min element in the tree
def AVLfindMinIter(root):
  curr = root
  while curr.left != None:
    curr = curr.left
  return curr
#Finding the successor of a node
def AVLfindNextIter(root, value):
  if root.right != None:
    return AVLfindMinIter(root.right)
  curr = root.parent
  while curr != None:
    if curr.value > root.value:
      return curr
  ...
  return None
#Finding the successor of a node
def AVLfindPrevIter(root, value):
  if root.right != None:
    return AVLfindMaxIter(root.right)
  curr = root.parent
  while curr != None:
    if curr.value > root.value:
      return curr
...
if __name__ == '__main__':
  BSTRoot = None
  AVLRoot = None
  randArray = getSortedArray(10000)
  for i in range(len(randArray)):
    AVLRoot = AVLinsertIter(AVLRoot, randArray[i])
